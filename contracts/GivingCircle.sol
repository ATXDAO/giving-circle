...
// CIRCLE LEADER FUNCTIONS
    ...
    function newCircle(uint _circlenumber) public returns (bool) {
        ...
        //need to work in ability to withdraw funds if circle doesn't go through
        //Will implement funding and funding checks at a later point
        // fundGiftForCircle(_circlenumber);
    ...
    function fundGiftForCircle(uint disburseforCircleNumber) public payable {
            ...
            emit FundedCircle(disburseforCircleNumber, USDCperCircle * weiMultiplier);
    }
    ...
    function disburseBeans(uint disburseforCircleNumber, address[] memory attendees) public virtual returns (bool) {
            ...
            require (
                circleNumbers[disburseforCircleNumber].step == 2, "Not right conditions to be disbursing beans!" // beans can only be disbursed once per circle
            );
    ...
    function closeCirclevoting(uint endcirclenumber) public virtual returns (bool) {
        ...
        require (
            circleNumbers[endcirclenumber].step == 5, "giving circle voting is not open"
        );
        
        // circleNumbers[endcirclenumber].step =  6;
        require(
            circleNumbers[endcirclenumber].circleFunded == true, "Before closing can end, you must have the circle funded"
        );
...
//giftRecipient = payableAddress
    ...
    function redeemGift(uint proposedincirclenumber, uint proposal) external virtual {
        ...
        require(
            circleNumbers[proposedincirclenumber].step == 6, "selected giving circle is still accepting votes"
        );
...
// BEAN HOLDER FUNCTIONS
    ...
    function placeBeans (uint circlenumb, uint propnumber, uint beanqty) external virtual returns (bool) {
        require (
            circleNumbers[circlenumb].step == 5, "giving circle is closed to voting"
        );
        require (
