...
// ADMIN FUNCTIONS
    ...
    // @tlogs: must be first function called be circle admin

    function firstAdminFunction() external returns (bool) {
        require (
            circleAdmin == msg.sender, "only circle leader can elect new leader"
        );
        newCircle(); // this will allign checkForCircle array mapping with circleNumbers
        newProposal(0, payable(msg.sender)); // this will allign checkForProp array mapping with proposalNumbers
        circleNumbers[0].step = 6; // set circle 0 to complete to allow for creation of circle one
        return true;
    }
...
// CIRCLE LEADER FUNCTIONS
    ...
    function newCircle() public returns (bool) {
     
        uint _newCircleNumber = checkforCircle.length; // sets new circle number to length of checkforCircle array, which is 1 after circle 0 created in constructor
        
        require(
           circleleader == msg.sender, "only circle leader can start a new circle"
        );
        require(
            circleExists(_newCircleNumber) == false, "giving circle already exists" // runs a for loop on checkforCircle array, will return true if duplicate
        );


        checkforCircle.push(_newCircleNumber); // add next circle number to the checkforCircle array.
        GivingCircle storage g = circleNumbers[_newCircleNumber]; // create a storage variable for giving circle to be created
        ...
        emit CircleCreated(_newCircleNumber);
...
//giftRecipient = payableAddress
    ...
    function newProposal(uint proposeInCircle, address payable giftRecipient) public virtual returns (bool) {
        uint _newPropNumber = checkforProp.length; // proposal 0 results in checkforProp.length of 1 after constructor, first proposal = 1
        
        require(
            proposalExists(_newPropNumber) == false, "gift proposal number already exists."
        );
        require(
        ...
        checkforProp.push(_newPropNumber); // add the gift proposal number to overall gift proposal check
        proposalincircle[_newPropNumber] = proposeInCircle; // sets mapping of 1-to-1 proposalincircle for quick lookup of what circle a proposal is in
        proposalsInCircle[proposeInCircle].push(_newPropNumber); // add the gift proposal to array within proposalsInCircle array within Giving Circle struct
        Proposal storage p = proposalNumbers[_newPropNumber]; // used to push Proposal Struct elements to proposalNumbers mapping below
        p.beansReceived = 0;
        p.giftAddress = giftRecipient;
        emit ProposalCreated(_newPropNumber, proposeInCircle, giftRecipient);
        return true;   
    }
...
// BEAN HOLDER FUNCTIONS
    ...
    function checkbeanBalance (address beanholder) external view returns (uint) {
